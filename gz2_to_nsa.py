...
gz2_specz[ndx].write('gz2_specz_nsa_only_{0}_arcsec.csv'.format(matching_radius.value), format='ascii.csv', overwrite=True)

print('match NSA to photoz')
ndx = cross_match(gz2_photoz)
gz2_photoz[ndx].write('gz2_photoz_nsa_only_{0}_arcsec.csv'.format(matching_radius.value), format='ascii.csv', overwrite=True)

print('match NSA to s82')
ndx = cross_match(gz2_s82)
gz2_s82[ndx].write('gz2_s82_nsa_only_{0}_arcsec.csv'.format(matching_radius.value), format='ascii.csv', overwrite=True)
